...
public class NullPattern
{
    ...
    public NullByte[] getPattern(byte[] data, int address, int mode){
            ...
            case "arm":
                ...
                for(int i = 0; i < insn.length; i++) {
                    if(insn[i].mnemonic.equals("ldr") || insn[i].mnemonic.equals("str")
                            || insn[i].mnemonic.equals("strb") || insn[i].mnemonic.equals("ldrb")
                            && Utils.hasTwoComma(insn[i].opStr)) {
                        for(int j = 0; j < 3; j++) {
                            pattern[kount] = new NullByte((byte)-1, true);
                            kount++;
                        }
                        pattern[kount] = new NullByte((byte)0xE5, false);
                        kount++;
                        continue;
                    }

                    if(insn[i].mnemonic.equals("add")) {
                        if(insn[i].opStr.contains("#")) {
                            for(int j = 0; j < 3; j++) {
                                pattern[kount] = new NullByte((byte)-1, true);
                                kount++;
                            }
                            pattern[kount] = new NullByte((byte)0xE2, false);
                            kount++;
                            continue;
                        }
                        else {
                            ...
                            pattern[kount] = new NullByte((byte)0xE0, false);
                            kount++;
                            continue;
                        }
                    }
        ...
        if(pattern.length < 0 || pattern[0] == null) {
            System.out.println("[NullLog] WARNING PROBABLY INVALID ADDRESS");
            return null;
        }
        ...
        switch(aarch) {
            ...
            case "arm":
                for (int i = 0; i < insn.length; i++) {
                    ...
                    if( insn[i].mnemonic.equals("bx") && insn[i].opStr.equals("lr")) {
                        return (int)insn[i].address + 0x4;
                    }
